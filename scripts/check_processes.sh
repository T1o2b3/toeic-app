#!/usr/bin/env bash
# Liệt kê tiến trình nền còn sống của project này. Dùng: bash scripts/check_processes.sh
# Chạy ở cuối mỗi bước con, trước khi báo "xong", để không bỏ quên máy chủ dev hay job dang dở.
FOUND=0

echo "— Máy chủ dev / tiến trình Vite:"
if pgrep -fl "vite" 2>/dev/null | grep -v grep; then FOUND=1; else echo "  (không có)"; fi

echo "— Pipeline đang chạy:"
if pgrep -fl "pipeline/" 2>/dev/null | grep -v grep; then FOUND=1; else echo "  (không có)"; fi

echo "— Cổng 5173 (dev server):"
if lsof -nP -iTCP:5173 -sTCP:LISTEN 2>/dev/null | tail -n +2; then FOUND=1; else echo "  (không ai giữ)"; fi

echo
if [ "$FOUND" -eq 1 ]; then
  echo "CÓ tiến trình nền đang sống. Nếu không còn cần, hãy tắt:"
  echo "  pkill -f vite          # tắt máy chủ dev"
  echo "  pkill -f 'pipeline/'   # dừng pipeline (cache đã lưu, chạy lại tiếp được)"
else
  echo "OK: không còn tiến trình nền nào."
fi
exit 0
